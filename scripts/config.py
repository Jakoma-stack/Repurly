...
ELIGIBLE_POSTING_STATUSES = {"approved", "generated", "queued", "scheduled"}
...
LINKEDIN_API_VERSION = os.getenv("LINKEDIN_API_VERSION", "202510").strip() or "202510"
LINKEDIN_CLIENT_ID = os.getenv("LINKEDIN_CLIENT_ID", "").strip()
LINKEDIN_CLIENT_SECRET = os.getenv("LINKEDIN_CLIENT_SECRET", "").strip()
LINKEDIN_REDIRECT_PATH = os.getenv("LINKEDIN_REDIRECT_PATH", "/linkedin/oauth/callback").strip() or "/linkedin/oauth/callback"
LINKEDIN_OAUTH_SCOPES = tuple(dict.fromkeys(item.strip() for item in os.getenv("LINKEDIN_OAUTH_SCOPES", "openid profile email w_member_social w_organization_social").split() if item.strip()))
...
APP_BASE_URL = os.getenv("APP_BASE_URL", "https://app.repurly.org").strip().rstrip("/")
...
WORKSPACE_INVITE_SEND_EMAILS = env_flag("WORKSPACE_INVITE_SEND_EMAILS", True)
PASSWORD_RESET_SEND_EMAILS = env_flag("PASSWORD_RESET_SEND_EMAILS", True)
TEAM_INVITE_SEND_EMAILS = env_flag("TEAM_INVITE_SEND_EMAILS", True)
...
ALLOWED_UPLOAD_EXTENSIONS = env_csv("ALLOWED_UPLOAD_EXTENSIONS", "png,jpg,jpeg,webp,gif,mp4,mov,m4v")
