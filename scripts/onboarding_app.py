            ...
            if existing_customer is not None:
                ...
                if workspace_id:
                    membership = conn.execute(
                        "SELECT 1 FROM workspace_memberships WHERE user_id=? AND workspace_id=? LIMIT 1",
                        (int(existing_customer["id"]), int(workspace_id)),
                    ).fetchone()
                    same_workspace = membership is not None
...
def _workspace_publishing_health(*, brands: list[dict[str, Any]], schedule_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    publishable_statuses = {"approved", "generated", "queued", "scheduled"}
...
def _friendly_publish_blockers(*, workspace_id: int, brands: list[dict[str, Any]]) -> list[dict[str, str]]:
    brand_names = {str(item.get("slug") or "").strip().lower(): str(item.get("display_name") or item.get("slug") or "Brand") for item in brands}
    if not brand_names:
        return []
    placeholders = ",".join(["?"] * len(brand_names))
    attempts = fetch_rows(
        f"SELECT brand_slug, error_message, updated_at FROM publish_attempts WHERE status='failed' AND lower(brand_slug) IN ({placeholders}) ORDER BY id DESC LIMIT 12",
        tuple(brand_names.keys()),
    )
    blockers: list[dict[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for attempt in attempts:
        slug = str(attempt.get("brand_slug") or "").strip().lower()
        raw = str(attempt.get("error_message") or "").strip()
        if not raw:
            continue
        title = "Posting needs attention"
        detail = raw
        action = "Open LinkedIn setup and review the posting connection."
        lower = raw.lower()
        if "member permissions must be used when using member as author" in lower:
            title = "Personal-profile posting is blocked by LinkedIn permissions"
            detail = "Your token is valid, but it does not include the member posting permission LinkedIn requires for personal-profile posting."
            action = "Reconnect LinkedIn with member posting permission (w_member_social) or switch this brand to company-page posting with a page admin token."
        elif "expects token from env var" in lower or "no linkedin access token available" in lower or "token" in lower and "missing" in lower:
            title = "Posting token is not available to the publisher"
            detail = "The content studio can see a token source, but the publish runner could not read a real token value when it tried to post."
            action = "Set the token source to LINKEDIN_ACCESS_TOKEN in the brand, then set LINKEDIN_ACCESS_TOKEN in both the web service and cron job environment variables."
        elif "not implemented yet" in lower:
            title = "This post format is not live-supported yet"
            detail = "The post is queued in a format the current publisher does not support live."
            action = "Use text-only or single-image posting for live publishing."
        elif "caption file" in lower or "asset files" in lower:
            title = "The scheduled post is missing required files"
            detail = "One or more files referenced by the scheduled post could not be found at publish time."
            action = "Open the scheduled draft, re-save it, and confirm the assets/caption are still attached."
        key=(slug,title)
        if key in seen:
            continue
        seen.add(key)
        blockers.append({
            "brand_name": brand_names.get(slug, slug or "Brand"),
            "title": title,
            "detail": detail,
            "action": action,
            "updated_at": str(attempt.get("updated_at") or "").strip(),
        })
    return blockers[:4]


def _content_queue_summary(schedule_rows: list[dict[str, Any]]) -> dict[str, int]:
    working = 0
    queued = 0
    due = 0
    failed = 0
    ...
    for row in schedule_rows:
        status = str(row.get("status") or "").strip().lower()
        if status in {"draft", "changes_requested", "pending"}:
            working += 1
        if status in {"approved", "generated", "queued", "scheduled", "posting"}:
            queued += 1
        if status == "failed":
            failed += 1
        if status in {"approved", "generated", "queued", "scheduled"} and schedule_row_is_due(row, now_utc=current):
            due += 1
    return {"working": working, "queued": queued, "due": due, "failed": failed}
...
@app.get("/workspace/help/download-guide")
@customer_login_required
def workspace_help_download_guide():
    workspace = current_workspace()
    if workspace is None:
        return redirect(url_for("customer_dashboard"))
    guide_path = Path(__file__).resolve().parents[1] / "docs" / "Repurly_Getting_Started_Guide.docx"
    if not guide_path.exists():
        abort(404)
    return send_file(
        guide_path,
        as_attachment=True,
        download_name="Repurly_Getting_Started_Guide.docx",
...
def workspace_content_page():
    ...
    queue_summary = _content_queue_summary(schedule_rows)
    publish_blockers = _friendly_publish_blockers(workspace_id=int(workspace["id"]), brands=brands)
    return render_template(
...
def workspace_content_run_due_check():
    ...
    try:
        ...
        summary = process_due_linkedin_posts(now_utc=datetime.now(timezone.utc), return_summary=True)
    except Exception as exc:
    ...
    posted = int(summary.get("posted_count") or 0)
    ...
    if eligible == 0:
        ...
        message = f"Publishing check ran successfully. {posted} due post{'s' if posted != 1 else ''} went out."
    elif posted and failed:
        message = f"Publishing check finished with mixed results: {posted} posted and {failed} failed. Review the blocker panel below."
    else:
        message = f"Publishing check finished, but none of the {eligible} due post{'s' if eligible != 1 else ''} could be sent. Review the blocker panel below."
    ...
    if request.method == "POST":
        ...
        if not errors:
            with get_conn() as conn:
                ...
                conn.commit()
            saved = "Your request has been saved. We will follow up using the details you provided."
