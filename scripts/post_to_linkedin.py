...
def build_linkedin_multiimage_payload(author_urn: str, text: str, image_urns: list[str], alt_texts: list[str] | None = None) -> dict[str, Any]:
    images: list[dict[str, Any]] = []
    alt_texts = alt_texts or []
    for index, image_urn in enumerate(image_urns):
        item: dict[str, Any] = {"id": image_urn}
        alt_text = (alt_texts[index] if index < len(alt_texts) else "").strip()
        if alt_text:
...
def build_linkedin_video_payload(author_urn: str, text: str, video_urn: str, title: str = "") -> dict[str, Any]:
...
def extract_linkedin_video_upload(data: dict[str, Any]) -> tuple[str, list[dict[str, Any]], str]:
    value = data.get("value") or data
    video_urn = str(value.get("video") or "").strip()
    upload_token = str(value.get("uploadToken") or "").strip()
    upload_instructions = value.get("uploadInstructions") or []
    if not video_urn or not upload_instructions:
        raise RuntimeError(f"Unexpected LinkedIn video initializeUpload response: {json.dumps(data)}")
    return video_urn, list(upload_instructions), upload_token


def register_linkedin_video_upload(token: str, owner_urn: str, video_path: Path) -> tuple[str, list[dict[str, Any]], str]:
    payload = {
        "initializeUploadRequest": {
            "owner": owner_urn,
    ...
    if not response.ok:
        raise RuntimeError(f"LinkedIn video initializeUpload error {response.status_code}: {response.text}")
    return extract_linkedin_video_upload(response.json())


def upload_linkedin_video_bytes(token: str, upload_instructions: list[dict[str, Any]], video_path: Path) -> list[str]:
    uploaded_part_ids: list[str] = []
    with video_path.open("rb") as file_obj:
        for instruction in upload_instructions:
            upload_url = str(instruction.get("uploadUrl") or "").strip()
            if not upload_url:
                raise RuntimeError(f"Missing LinkedIn video uploadUrl in instruction: {json.dumps(instruction)}")
            first_byte = int(instruction.get("firstByte", 0) or 0)
            last_byte = int(instruction.get("lastByte", -1) or -1)
            if last_byte < first_byte:
                raise RuntimeError(f"Invalid LinkedIn video upload range: {json.dumps(instruction)}")
            file_obj.seek(first_byte)
            chunk = file_obj.read(last_byte - first_byte + 1)
            ...
            if response.status_code not in {200, 201}:
                raise RuntimeError(f"LinkedIn video upload error {response.status_code}: {response.text}")
            uploaded_part_ids.append((response.headers.get("etag") or response.headers.get("ETag") or "").strip() or upload_url.split("?")[0])
    return uploaded_part_ids


def finalize_linkedin_video_upload(token: str, video_urn: str, upload_token: str, uploaded_part_ids: list[str]) -> None:
    payload = {
    ...
    if not response.ok:
        raise RuntimeError(f"LinkedIn video finalizeUpload error {response.status_code}: {response.text}")


def fetch_linkedin_video_status(token: str, video_urn: str) -> dict[str, Any]:
    ...
    if not response.ok:
        raise RuntimeError(f"LinkedIn video status error {response.status_code}: {response.text}")
    return response.json()
...
def _video_status(video_status: dict[str, Any]) -> str:
    return str(video_status.get("status") or "").upper()
...
def wait_for_linkedin_video_ready(token: str, video_urn: str, *, attempts: int = 20, sleep_seconds: float = 3.0) -> dict[str, Any]:
    last_status: dict[str, Any] = {}
    for _ in range(attempts):
        last_status = fetch_linkedin_video_status(token, video_urn)
        status = _video_status(last_status)
        if status == "AVAILABLE":
            return last_status
        if status == "PROCESSING_FAILED":
            raise RuntimeError(f"LinkedIn video processing failed: {json.dumps(last_status)}")
        time.sleep(sleep_seconds)
    raise RuntimeError(f"LinkedIn video did not become ready in time: {json.dumps(last_status)}")
...
def _expand_alt_texts(image_alt_text: str, asset_count: int) -> list[str]:
    raw = (image_alt_text or "").strip()
    if not raw:
        return [""] * asset_count
    chunks = [chunk.strip() for chunk in raw.replace("\n", "|").replace(";", "|").split("|") if chunk.strip()]
    if len(chunks) >= asset_count:
        return chunks[:asset_count]
    if len(chunks) == 1:
        return [chunks[0] for _ in range(asset_count)]
    return chunks + [chunks[-1]] * max(0, asset_count - len(chunks))
...
def publish_image_post(brand: str, text: str, asset_paths: list[Path], image_alt_text: str = "") -> dict[str, Any]:
    ...
    alt_texts = _expand_alt_texts(image_alt_text, len(asset_paths))
    image_urns: list[str] = []
    ...
    if mode == "single_image":
        payload = build_linkedin_single_image_payload(author_urn, text, image_urns[0], alt_texts[0] if alt_texts else "")
    else:
        payload = build_linkedin_multiimage_payload(author_urn, text, image_urns, alt_texts)
    response = create_linkedin_post(access_token, payload)
    if not response.ok:
        raise RuntimeError(f"LinkedIn API error {response.status_code}: {response.text}")
    return {
        ...
        "author_urn": author_urn,
        "asset_mode": mode,
        "asset_urns": image_urns,
        "asset_urn": image_urns[0] if image_urns else "",
        "platform_post_id": extract_linkedin_post_id_from_response(response),
        "status_code": response.status_code,
        "response_text": response.text,
    }
...
def publish_video_post(brand: str, text: str, asset_path: Path, title: str = "") -> dict[str, Any]:
    author_urn, access_token = resolve_linkedin_credentials(brand)
    if not asset_path.exists():
        raise FileNotFoundError(f"Missing asset file: {asset_path.as_posix()}")
    if linkedin_dry_run_enabled():
        return {
            "ok": True,
            "dry_run": True,
            ...
            "asset_mode": "video",
            "platform_post_id": "",
            "assets": [asset_path.as_posix()],
            "message": "Dry run enabled. Video upload not attempted.",
        }
    video_urn, upload_instructions, upload_token = register_linkedin_video_upload(access_token, author_urn, asset_path)
    uploaded_part_ids = upload_linkedin_video_bytes(access_token, upload_instructions, asset_path)
    finalize_linkedin_video_upload(access_token, video_urn, upload_token, uploaded_part_ids)
    wait_for_linkedin_video_ready(access_token, video_urn)
    payload = build_linkedin_video_payload(author_urn, text, video_urn, title=title or asset_path.stem.replace("_", " ").strip())
    response = create_linkedin_post(access_token, payload)
    if not response.ok:
        raise RuntimeError(f"LinkedIn API error {response.status_code}: {response.text}")
    return {
...
def _process_selected_linkedin_rows(schedule_df, eligible_df):
    ...
    for _, row in eligible_df.iterrows():
        ...
        try:
            ...
            elif validation_context["asset_mode"] == "video":
                result = publish_video_post(
                    brand,
                    validation_context["caption_text"],
                    validation_context["asset_paths"][0],
                    title=(row.get("notes") or row.get("title") or ""),
                )
            else:
...
def process_due_linkedin_posts(*, now_utc: datetime | None = None, return_summary: bool = False) -> int | dict[str, Any]:
    schedule_df = normalise_optional_columns(load_csv(SCHEDULE_CSV), OPTIONAL_SCHEDULE_COLUMNS)
    current = now_utc or datetime.now(timezone.utc)
    ...
    if eligible_df.empty:
        ...
        return {"eligible_count": 0, "posted_count": 0, "failed_count": 0, "exit_code": 0} if return_summary else 0

    schedule_df, summary = _process_selected_linkedin_rows(schedule_df, eligible_df)
    save_csv(schedule_df, SCHEDULE_CSV)
    exit_code = 0 if summary["failed_count"] == 0 else 1
    logger.info("LinkedIn due-post processing complete | posted=%s | failed=%s", summary["posted_count"], summary["failed_count"])
    if return_summary:
        summary = {**summary, "eligible_count": len(eligible_df), "exit_code": exit_code}
        return summary
    return exit_code
