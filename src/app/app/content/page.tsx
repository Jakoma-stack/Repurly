...
import { generateAiDrafts, requestApproval, saveDraft, schedulePost } from '@/server/actions/workflow';
import { getLinkedInTargets, getPostForEditing, getRecentPosts, getWorkspaceBrandOptions } from '@/server/queries/workflow';
...
export default async function ContentPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [targets, brandOptions, editingPost, recentPosts] = await Promise.all([
  ...
  const selectedBrandId = editingPost?.brandId ?? brandOptions.find((brand) => brand.status !== 'archived')?.id ?? brandOptions[0]?.id ?? '';
  ...
  return (
    <div className="space-y-6">
      ...
      {ok === 'generated' && <Banner kind="success">AI draft batch created. Review the drafts below and tighten the best one.</Banner>}
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Pull the AI drafting workflow from the older build into the current interface, but keep it narrow: LinkedIn-first,
            buyer-focused, and draft-only.
          </p>
        </CardHeader>
        <CardContent>
          <form action={generateAiDrafts} className="grid gap-4 xl:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="space-y-4 rounded-3xl border border-border p-5">
              ...
              <div>
                ...
                <textarea
                  name="brief"
                  className="mt-2 min-h-[140px] w-full rounded-2xl border border-border px-4 py-3 text-sm"
                  defaultValue="Write three LinkedIn post ideas about why B2B teams should keep content workflows narrow, reliable, and commercially useful."
                  required
                />
              </div>
            </div>
            <div className="space-y-4 rounded-3xl border border-border p-5">
              <div>
                ...
                <input name="commercialGoal" className="mt-2 w-full rounded-2xl border border-border px-4 py-3 text-sm" defaultValue="Book qualified pilot conversations" />
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
                <div>
                  <label className="text-sm font-medium text-slate-900">Draft count</label>
                  <input name="count" type="number" min={1} max={6} defaultValue={3} className="mt-2 w-full rounded-2xl border border-border px-4 py-3 text-sm" />
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-900">Format</label>
                  <select name="postFormat" defaultValue="text" className="mt-2 w-full rounded-2xl border border-border px-4 py-3 text-sm">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Finish the core workflow: choose a brand, draft the post, select the LinkedIn target, request approval, then schedule.
          </p>
        </CardHeader>
        <CardContent>
          <form className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="space-y-4">
              <div className="rounded-3xl border border-border p-5">
                ...
                <p className="mt-1 text-sm text-muted-foreground">LinkedIn remains the hero channel. Pick the profile or company page that should receive this post.</p>
