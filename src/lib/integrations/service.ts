...
export async function syncPlatformAccounts(
  ...
  const newAccounts = accounts.filter((account) => {
    return !existing.some((row) => row.externalAccountId === account.handle || row.handle === account.handle);
  });

  if (newAccounts.length) {
    const limit = await checkPlanLimit(workspaceId, 'connectedChannels', newAccounts.length);
    if (!limit.allowed) {
      throw new Error(`PLAN_LIMIT_EXCEEDED:connectedChannels:${limit.plan}:${limit.used}/${limit.limit}`);
    }
  }
