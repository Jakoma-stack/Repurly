...
async function createOrUpdateBasePost(formData: FormData, status: 'draft' | 'in_review' | 'scheduled') {
  ...
  const planLimit = await checkPlanLimit(workspaceId, 'monthlyPosts');
  if (!planLimit.allowed) return { error: 'plan-limit-posts' as const, post: null };
...
export async function generateAiDrafts(formData: FormData) {
  ...
  const draftPlanLimit = await checkPlanLimit(workspaceId, 'monthlyPosts', count);
  if (!draftPlanLimit.allowed) {
    redirect(buildContentPath({ error: 'plan-limit-posts' }, 'campaign-planner') as Route);
  }
