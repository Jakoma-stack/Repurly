...
export async function getSettingsSnapshot(workspaceId: string) {
  ...
  let members: Array<{ id: string; clerkUserId: string; role: string; createdAt: Date | null }> = [];
  let invites: Array<{ id: string; email: string; role: string; status: string; token: string; createdAt: Date | null }> = [];

  try {
    flags = await getWorkspaceOperatorFlags(workspaceId);
  } catch {
    flags = emptyFlags;
  }
  ...
  try {
    ...
    // fall back to zeros so Settings can still load
  }

  try {
    members = await db
      .select({ id: workspaceMemberships.id, clerkUserId: workspaceMemberships.clerkUserId, role: workspaceMemberships.role, createdAt: workspaceMemberships.createdAt })
  ...
  try {
    invites = await db
      .select({ id: workspaceInvites.id, email: workspaceInvites.email, role: workspaceInvites.role, status: workspaceInvites.status, token: workspaceInvites.token, createdAt: workspaceInvites.createdAt })
  ...
  return { flags, supportSnapshot, members, invites };
}
