...
export const dailyAgentSessions = pgTable("daily_agent_sessions", {
  id: uuid("id").defaultRandom().primaryKey(),
  workspaceId: uuid("workspace_id").references(() => workspaces.id, { onDelete: "cascade" }).notNull(),
  brandId: uuid("brand_id").references(() => brands.id, { onDelete: "set null" }),
  sessionDate: varchar("session_date", { length: 10 }).notNull(),
  rawNotifications: text("raw_notifications"),
  rawComments: text("raw_comments"),
  rawAnalytics: text("raw_analytics"),
  analyticsImportJson: jsonb("analytics_import_json").$type<Record<string, unknown>>(),
  rawProfiles: text("raw_profiles"),
  rawNotes: text("raw_notes"),
  summary: text("summary"),
  briefingJson: jsonb("briefing_json").$type<Record<string, unknown>>(),
  generationMode: varchar("generation_mode", { length: 24 }).notNull().default("fallback"),
  status: varchar("status", { length: 24 }).notNull().default("generated"),
  usefulnessRating: varchar("usefulness_rating", { length: 24 }),
...
export const dailyAgentActions = pgTable("daily_agent_actions", {
  id: uuid("id").defaultRandom().primaryKey(),
  sessionId: uuid("session_id").references(() => dailyAgentSessions.id, { onDelete: "cascade" }).notNull(),
  workspaceId: uuid("workspace_id").references(() => workspaces.id, { onDelete: "cascade" }).notNull(),
  brandId: uuid("brand_id").references(() => brands.id, { onDelete: "set null" }),
  actionType: varchar("action_type", { length: 48 }).notNull(),
  personName: varchar("person_name", { length: 160 }),
  personHandle: varchar("person_handle", { length: 160 }),
  source: varchar("source", { length: 80 }).notNull().default("daily_agent"),
  recommendedChannel: varchar("recommended_channel", { length: 48 }).notNull().default("tracker_update"),
  priority: varchar("priority", { length: 16 }).notNull().default("medium"),
  reason: text("reason"),
  draftText: text("draft_text"),
  status: varchar("status", { length: 24 }).notNull().default("draft"),
  linkedLeadId: uuid("linked_lead_id").references(() => leadPipeline.id, { onDelete: "set null" }),
  metadata: jsonb("metadata").$type<Record<string, unknown>>(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
