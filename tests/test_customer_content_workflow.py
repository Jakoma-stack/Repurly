...
def test_ai_generation_produces_real_caption_copy(content_client):
    ...
    assert "learn more" in caption.lower() or "book a discovery call" in caption.lower()
...
def test_help_page_offers_downloadable_guide(content_client):
    client, *_ = content_client
    response = client.get("/workspace/help")
    assert response.status_code == 200
    assert b"Download getting started guide" in response.data


def test_help_guide_can_be_downloaded(content_client):
    client, *_ = content_client
    response = client.get("/workspace/help/download-guide")
    assert response.status_code == 200
    assert "attachment;" in response.headers.get("Content-Disposition", "")
    assert response.headers.get("Content-Type", "").startswith("application/vnd.openxmlformats-officedocument.wordprocessingml.document")
