...
def test_build_linkedin_multiimage_payload_includes_all_images():
    payload = linkedin.build_linkedin_multiimage_payload(
        "urn:li:person:test",
        "hello",
    ...
    assert [item["id"] for item in images] == ["urn:li:image:1", "urn:li:image:2"]
    assert images[1]["altText"] == "Two"


def test_publish_carousel_post_live_flow(monkeypatch, tmp_path):
    monkeypatch.setenv("LINKEDIN_DRY_RUN", "false")
    monkeypatch.setenv("LINKEDIN_ACCESS_TOKEN_JAKOMA", "brand-token")

    image_one = tmp_path / "one.png"
    image_two = tmp_path / "two.png"
    image_one.write_bytes(b"png1")
    image_two.write_bytes(b"png2")

    urns = iter([
        ("urn:li:image:1", "https://upload.example.com/1"),
        ("urn:li:image:2", "https://upload.example.com/2"),
    ])
    monkeypatch.setattr(linkedin, "register_linkedin_image_upload", lambda token, owner: next(urns))
    monkeypatch.setattr(linkedin, "upload_linkedin_image_bytes", lambda token, url, path: None)
    monkeypatch.setattr(linkedin, "wait_for_linkedin_asset_ready", lambda token, asset_urn: {"status": "AVAILABLE"})
    monkeypatch.setattr(linkedin, "create_linkedin_post", lambda token, payload: DummyResponse(ok=True, status_code=201, text="created", json_data={"id": "urn:li:share:123"}))

    result = linkedin.publish_image_post("jakoma", "hello", [image_one, image_two], "Slide one|Slide two")

    assert result["asset_mode"] == "carousel"
    assert result["asset_urns"] == ["urn:li:image:1", "urn:li:image:2"]
    assert result["platform_post_id"] == "urn:li:share:123"


def test_publish_video_post_live_flow(monkeypatch, tmp_path):
    monkeypatch.setenv("LINKEDIN_DRY_RUN", "false")
    monkeypatch.setenv("LINKEDIN_ACCESS_TOKEN_JAKOMA", "brand-token")

    video_path = tmp_path / "teaser.mp4"
    video_path.write_bytes(b"0" * 16)

    monkeypatch.setattr(linkedin, "register_linkedin_video_upload", lambda token, owner, path: ("urn:li:video:1", [{"uploadUrl": "https://upload.example.com/video", "firstByte": 0, "lastByte": 15}], "token-123"))
    monkeypatch.setattr(linkedin, "upload_linkedin_video_bytes", lambda token, instructions, path: ["part-1"])
    finalized = {}
    monkeypatch.setattr(linkedin, "finalize_linkedin_video_upload", lambda token, video_urn, upload_token, uploaded_part_ids: finalized.update({"video": video_urn, "token": upload_token, "parts": uploaded_part_ids}))
    monkeypatch.setattr(linkedin, "wait_for_linkedin_video_ready", lambda token, video_urn: {"status": "AVAILABLE"})
    monkeypatch.setattr(linkedin, "create_linkedin_post", lambda token, payload: DummyResponse(ok=True, status_code=201, text="created", json_data={"id": "urn:li:share:456"}))

    result = linkedin.publish_video_post("jakoma", "hello", video_path, title="Demo reel")

    assert result["asset_mode"] == "video"
    assert result["asset_urn"] == "urn:li:video:1"
    assert finalized == {"video": "urn:li:video:1", "token": "token-123", "parts": ["part-1"]}
