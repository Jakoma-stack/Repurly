...
def test_public_root_redirects_to_start_and_health_routes_render(app_client):
    client, _, _ = app_client

    root_response = client.get("/", follow_redirects=False)
    assert root_response.status_code == 302
    assert root_response.headers["Location"].endswith("/start")
...
def test_start_signup_requires_privacy_consent(app_client):
